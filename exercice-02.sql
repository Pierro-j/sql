...
FROM tag
WHERE name = 'SQL'
UPDATE tag SET description = 'structured query language' WHERE name = 'SQL';
--
--
...
UPDATE tag
SET description = 'FStructured Query Language'
WHERE id = 6
...
-- - nom : SASS
-- - description : aucune
INSERT INTO tag (name, description) VALUES ('SASS', '');
-- Exo 2.2
...
INSERT INTO student_tag (student_id, tag_id) VALUES (1, (SELECT id FROM tag WHERE name = 'SASS'));

-- Exo 2.4
-- Mettez à jour le tag `HTML` :
-- description : Le HTML est un language de structuration de données.
SELECT * FROM tag WHERE name = 'Html' ;
UPDATE tag SET description = 'Le HTML est un language de structuration de données.' WHERE id = 1;
--! (Ou bien) UPDATE tag SET description = 'Le HTML est un language de structuration de données.' WHERE name = 'HTML';
-- Exo 2.5
...
UPDATE tag SET description = "Le terminal Bash est l\'interface en ligne de commande par défaut sous Linux." WHERE id = 1;
...
DELETE FROM student_tag WHERE student_id = (SELECT id FROM student WHERE firstname = 'Zacharie' AND lastname = 'Evrard')  AND tag_id = (SELECT id FROM tag WHERE name = 'Terminal Bash');
...
-- - student : Odette Thomas
DELETE from student WHERE firstname = 'Odette' AND lastname = 'Thomas'
