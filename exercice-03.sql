...
SELECT * FROM student WHERE email = '%.fr' OR created_at <= 2021-01-10

-- Exo 3.2
-- Listez les students :
-- - dont l'email contient la chaîne de caractères `.fr`
-- - ET dont la date de création est antérieure au 10 janvier 2021 inclus
SELECT * FROM student WHERE email = '%.fr' AND created_at <= 2021-01-10
...
-- - OU ayant un project
SELECT * FROM student WHERE email = '%.fr' AND created_at <= '2021-01-10' OR project_id IS NOT NULL;
...
SELECT * FROM student WHERE email = '%.fr' AND created_at <= '2021-01-10' AND project_id IS NULL
